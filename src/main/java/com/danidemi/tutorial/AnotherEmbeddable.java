 ...
 * Highly inspired by the embeddable {@linkplain http://timewalker74.github.io/ffmq/doc.html#III} provided in the FFMQ doc.
 */
public class AnotherEmbeddable implements EmbeddableServer
{
	private static final Logger log = LoggerFactory.getLogger(AnotherEmbeddable.class);
    private FFMQEngine engine;
    private String engineName;
    private TcpListenerDescriptor tcpListener;
    private List<QueueDefinition> queueDefinitions;
    private List<TopicDefinition> topicDefinitions;
...
//		this.destinationDefinitions = new ArrayList<AbstractDestinationDefinition>();
...
//		destinationDefinitions.addAll( topicDefinitions );
	...
	public void start() throws ServerException {
		...
		if(ffmqThread != null) throw new ServerException("Already running.");
			...
			public void run() {
		        try
		        {
		            ...
		            for (QueueDefinition queueDefinition : queueDefinitions) {
		            	if (!engine.getDestinationDefinitionProvider().hasQueueDefinition( queueDefinition.getName() )){
		            		...
		            		if(queueDefinition.getDataFolder() == null){
		            			queueDefinition.setDataFolder( engine.getSetup().getDefaultDataDir() );
		            		}
		            ...
		            for (TopicDefinition topicDefinition : topicDefinitions) {
		            	if (!engine.getDestinationDefinitionProvider().hasTopicDefinition(topicDefinition.getName() )){
		            		
		            		if(topicDefinition.getDataFolder() == null){
		            			topicDefinition.setDataFolder( engine.getSetup().getDefaultDataDir() );
		            		}
	...
	public void stop() throws ServerException {
		...
		try {
			...
			new ServerException("An exception occurred while waiting the FFMQ thread to stop.", e);
		}
		if(run == true){
			new ServerException("Failed to stop in a reasonable amount of time.");
		}
	}
	...
	public static void main(String[] args) {
		...
		List<QueueDefinition> mqueueDefinitions = Arrays.asList(
				queue1
		);
		...
		List<TopicDefinition> mtopicDefinitions = Arrays.asList(
				topic1
		);
		
		Properties props = new Properties();
		
//		props.setProperty(FFMQCoreSettings.DESTINATION_DEFINITIONS_DIR, destinationDefinitionsDirectory.getAbsolutePath());
//		props.setProperty(FFMQCoreSettings.TEMPLATES_DIR, templatesDirectory.getAbsolutePath());
//		props.setProperty(FFMQCoreSettings.TEMPLATE_MAPPING_FILE, templateMapping.getAbsolutePath());
//		props.setProperty(FFMQCoreSettings.DEFAULT_DATA_DIR, defaultDataDirectory.getAbsolutePath());
		
		
		
		Properties properties = new SingleFolderConfiguration( new File(FileUtils.getTempDirectory(), "ffmq") ).getProperties();
		Settings mengineSettings = new Settings( properties );
		
		TcpListenerDescriptor mtcpListener = new TcpListenerDescriptor("0.0.0.0", 10002, new Settings());
		AnotherEmbeddable emb = new AnotherEmbeddable(mengineName, mtcpListener, mqueueDefinitions, mtopicDefinitions, mengineSettings);
